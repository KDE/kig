...
public:
   ...
   * Returns an invalid Coordinate.  This is a special value of a
  ...
  /** Length
   * Returns the length or norm of this coordinate.  I.e. return the
   * distance from this Coordinate to the origin.
   * \see squareLength
   */
  double length () const;
  /** Square length
   * Equivalent to the square of \ref length, but a bit more efficient
   * because no square root has to be calculated.
   * \see length
   */
  ...
  /** Round
   * Returns this coordinate, rounded down to the nearest integral
   * values.
   */
