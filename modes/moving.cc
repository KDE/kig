...
void MovingModeBase::initScreen(const std::vector<ObjectCalcer *> &in)
{
    ...
    for (std::vector<ObjectHolder *>::iterator i = docobjs.begin(); i != docobjs.end(); ++i)
    ...
    std::set_difference(docobjsset.begin(), docobjsset.end(), drawableset.begin(), drawableset.end(), std::inserter(notmovingobjs, notmovingobjs.begin()));

    mview.clearStillPix();
    KigPainter p(mview.screenInfo(), &mview.stillPix, mdoc.document());
    p.drawGrid(mdoc.document().coordinateSystem(), mdoc.document().grid(), mdoc.document().axes());
...
void MovingModeBase::leftReleased(QMouseEvent *, KigWidget *v)
{
    ...
    for (std::vector<ObjectCalcer *>::iterator i = mcalcable.begin(); i != mcalcable.end(); ++i)
...
MovingMode::MovingMode(const std::vector<ObjectHolder *> &os, const Coordinate &c, KigWidget &v, KigPart &doc)
    : MovingModeBase(doc, v)
    , d(new Private)
{
    d->pwwsm = c;
    std::vector<ObjectCalcer *> emo;
...
{
    QString text = d->emo.size() == 1 ? d->emo[0]->imp()->type()->moveAStatement() : i18np("Move %1 Object", "Move %1 Objects", d->emo.size());
...
void MovingMode::moveTo(const Coordinate &o, bool snaptogrid)
{
    for (std::vector<ObjectCalcer *>::iterator i = d->emo.begin(); i != d->emo.end(); ++i) {
        ...
        if (snaptogrid)
            nc = mdoc.document().coordinateSystem().snapToGrid(nc, mview);
...
{
    Coordinate realo = snaptogrid ? mdoc.document().coordinateSystem().snapToGrid(o, mview) : o;
    ObjectFactory::instance()->redefinePoint(static_cast<ObjectTypeCalcer *>(mp->calcer()), realo, mdoc.document(), mview);
}
...
void PointRedefineMode::stopMove()
{
    ...
    std::vector<ObjectCalcer::shared_ptr> newparentsref(newparents.begin(), newparents.end());
    const ObjectType *newtype = mpcalc->type();

    std::vector<ObjectCalcer *> oldparents;
    for (std::vector<ObjectCalcer::shared_ptr>::iterator i = moldparents.begin(); i != moldparents.end(); ++i)
    ...
    command->addTask(new ChangeParentsAndTypeTask(mpcalc, newparents, newtype));
    mmon->finish(command);
