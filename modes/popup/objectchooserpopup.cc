...
int ObjectChooserPopup::getObjectFromList(const QPoint &p, KigWidget *w, const std::vector<ObjectHolder *> &objs, bool givepopup)
{
    int size = objs.size();
    ...
    for (std::vector<ObjectHolder *>::const_iterator i = objs.begin(); i != objs.end(); ++i) {
        if ((*i)->imp()->inherits(PointImp::stype()))
            numpoints++;
        else if ((*i)->imp()->inherits(FilledPolygonImp::stype()))
            numpolygons++;
        else
            numothers++;
    }
    ...
    if ((numpoints == 1) || ((numpoints == 0) && (numothers == 1)) || (size == 1))
        id = 0;
    else {
        if (givepopup) {
...
ObjectChooserPopup::ObjectChooserPopup(const QPoint &p, KigWidget &view, const std::vector<ObjectHolder *> &objs)
    : QMenu()
    , mplc(p)
    , mview(view)
    , mobjs(objs)
    , mselected(-1)
{
    ...
    for (uint i = 0; i < mobjs.size(); i++) {
        newaction = addAction(!mobjs[i]->name().isEmpty() ? QStringLiteral("%1 %2").arg(mobjs[i]->imp()->type()->translatedName()).arg(mobjs[i]->name())
