...
void BaseMode::leftMouseMoved(QMouseEvent *e, KigWidget *w)
{
    if (!moco.empty() && (mplc - e->pos()).manhattanLength() > 3)
        dragObject(moco, mplc, *w, (e->modifiers() & (Qt::ShiftModifier | Qt::ControlModifier)) != 0);
}
...
{
    if ((e->pos() - mplc).manhattanLength() > 4)
        return;
...
void BaseMode::leftClickedObject(ObjectHolder *, const QPoint &, KigWidget &, bool)
{
}

void BaseMode::dragObject(const std::vector<ObjectHolder *> &, const QPoint &, KigWidget &, bool)
{
}
