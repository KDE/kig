...
AngleImp::AngleImp(const Coordinate &pt, double start_angle_in_radials, double angle_in_radials, bool markRightAngle)
    : mpoint(pt)
    , mstartangle(start_angle_in_radials)
    , mangle(angle_in_radials)
    , mmarkRightAngle(markRightAngle)
{
}
...
ArcImp::ArcImp(const Coordinate &center, const double radius, const double startangle, const double angle)
    : CurveImp()
    , mcenter(center)
    , mradius(radius)
    , msa(startangle)
...
double ArcImp::getParam(const Coordinate &c, const KigDocument &) const
{
    ...
    angle /= ma;
    if (mradius < 0)
        angle = 1.0 - angle; // this is to avoid abrupt jumps when an ArcBTPType changes concavity
    return angle;
}
...
{
    if (mradius < 0)
        p = 1.0 - p; // this is to avoid abrupt jumps when an ArcBTPType changes concavity
...
{
    return rhs.inherits(ArcImp::stype()) && static_cast<const ArcImp &>(rhs).radius() == radius()
        && static_cast<const ArcImp &>(rhs).startAngle() == startAngle() && static_cast<const ArcImp &>(rhs).angle() == angle();
}

bool AngleImp::equals(const ObjectImp &rhs) const
{
    return rhs.inherits(AngleImp::stype()) && static_cast<const AngleImp &>(rhs).point() == point()
        && static_cast<const AngleImp &>(rhs).startAngle() == startAngle() && static_cast<const AngleImp &>(rhs).angle() == angle();
}

bool VectorImp::equals(const ObjectImp &rhs) const
{
    return rhs.inherits(VectorImp::stype()) && static_cast<const VectorImp &>(rhs).a() == a() && static_cast<const VectorImp &>(rhs).b() == b();
}
