...
double CubicImp::getParam(const Coordinate &p, const KigDocument &) const
{
    ...
    double f =
        a000 + a001 * x + a002 * y + a011 * x * x + a012 * x * y + a022 * y * y + a111 * x * x * x + a112 * x * x * y + a122 * x * y * y + a222 * y * y * y;
    if (f != 0) {
        ...
        if (f < 0)
            v = -v; // the line points away from the intersection
        double a, b, c, d;
        ...
        bool valid;
        int numroots;
        double lambda = calcCubicRoot(-1e10, 1e10, a, b, c, d, variations, valid, numroots);
        if (valid) {
...
const Coordinate CubicImp::getPoint(double p) const
{
    ...
    int numroots;
    bool valid = true;
    double y = calcCubicYvalue(x, -double_inf, double_inf, root, mdata, valid, numroots);
    if (!valid)
...
{
    return rhs.inherits(CubicImp::stype()) && static_cast<const CubicImp &>(rhs).data() == data();
}
...
const ObjectImpType *CubicImp::stype()
{
    static const ObjectImpType t(Parent::stype(),
                                 "cubic",
                                 kli18n("cubic curve"),
                                 kli18n("Select this cubic curve"),
                                 kli18n("Select cubic curve %1"),
...
{
    return (fabs(mdata.coeffs[9]) < 1e-12 && // y^3
