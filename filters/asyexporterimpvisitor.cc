...
void AsyExporterImpVisitor::emitLine(const Coordinate &a, const Coordinate &b, const int width, const Qt::PenStyle s, bool vector)
{
    mstream << "path line = " << emitCoord(a) << "--" << emitCoord(b) << ";";
    newLine();

    if (vector == true) {
        mstream << "draw(line, " << emitPen(mcurobj->drawer()->color(), width, s) << ", Arrow );";
    } else {
        mstream << "draw(line, " << emitPen(mcurobj->drawer()->color(), width, s) << " );";
    }
    newLine();
}
...
void AsyExporterImpVisitor::plotGenericCurve(const CurveImp *imp)
{
    ...
    for (uint i = 0; i < coordlist.size(); ++i) {
        ...
        mstream << "draw(curve, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << " );";
        newLine();
    }
}
...
{
    mstream << "pair point = " << emitCoord(imp->coordinate()) << ";";
    newLine();
    ...
    if (width == -1)
        width = 6;
    mstream << "dot(point, " << emitPen(mcurobj->drawer()->color(), width, mcurobj->drawer()->style()) << ");";
    newLine();
}
...
void AsyExporterImpVisitor::visit(const AngleImp *imp)
{
    ...
    mstream << "path a = Arc(" << emitCoord(center) << ", " << radius << ", " << startangle << ", " << endangle << " );";
    newLine();
    mstream << "draw(a, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << ", Arrow );";
    newLine();
}
...
{
    mstream << "pair center = " << emitCoord(imp->center()) << ";";
    newLine();
    mstream << "real radius = " << imp->radius() << ";";
    newLine();
    mstream << "path circle = Circle(center, radius);";
    newLine();
    mstream << "draw(circle, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << " );";
    newLine();
}
...
void AsyExporterImpVisitor::visit(const ArcImp *imp)
{
    ...
    mstream << "path arc = Arc(" << emitCoord(center) << ", " << radius << ", " << startangle << ", " << endangle << " );";
    newLine();
    mstream << "draw(arc, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << " );";
    newLine();
}
...
void AsyExporterImpVisitor::visit(const FilledPolygonImp *imp)
{
    ...
    mstream << "cycle;";
    newLine();
    mstream << "fill(polygon, " << emitPenColor(mcurobj->drawer()->color()) << "+opacity(0.5) );";
    newLine();
    mstream << "draw(polygon, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << " );";
    newLine();
}
...
void AsyExporterImpVisitor::visit(const BezierImp *imp)
{
    ...
    case 3:
        ...
        mstream << ";";
        newLine();
        mstream << "draw(bezier, " << emitPen(mcurobj->drawer()->color(), mcurobj->drawer()->width(), mcurobj->drawer()->style()) << " );";
        newLine();
        break;
    case 4:
        mstream << "path bezier = ";
