...
#ifdef CABRI_DEBUG
  kdDebug() << k_funcinfo << "which: " << which << endl;
#endif
  if ( nlines == 2 )
  ...
  while ( ! f.atEnd() )
  {
    ...
    for ( std::vector<int>::iterator i = obj->parents.begin();
      ...
      if ( *i <= static_cast<int>( lowerbound ) )
      {
        ...
        {
          kdDebug() << ">>>>>>>>> OBJECT TO AXIS/GRID: \"" << obj->type << "\"" << endl;
        }
#endif
      }
      else
    ...
    if ( args.size() > 0 )
      ...
      {
        kdDebug() << "+++++++++ arg[" << j << "]: " << args[j] << " - "
                  << args[j]->imp()->type()->internalName() << endl;
      }
    else
      kdDebug() << "+++++++++ args: NO" << endl;
    kdDebug() << "+++++++++ data: " << obj->data.size() << endl;
#endif
    if ( obj->type == "Pt" )
    {
